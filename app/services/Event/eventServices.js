...
module.exports = {
    ...
    getEvents:async()=> {
        return await Event.find({deleted_by:null}).sort({createdAt:-1})
    },

    getEvent:async(body)=> {
        const {eventId} = body;
        return await Event.find({_id:eventId, deleted_by:null})
    },
    ...
    deleteEvent:async(body)=> {
        ...
        await Event.findOneAndUpdate({created_by:userId, _id:eventId, deleted_by:null}, {deleted_by:userId});
    }
}